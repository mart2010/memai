...
| **Concept engagement gate** | A brand-new *organic* concept (no match, authored or organic) needs at least two of the conversation's own user turns topically similar to it (embedding-based) before it's inserted — an assistant-only mention is not enough (FR-310). |
