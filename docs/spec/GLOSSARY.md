...
| **Conversation boundary** | Whether the current exchange continues or breaks from the previous topic: `[TOPIC_BREAK]` (split; new conversation) or `[TOPIC_CONTINUATION]` (first turn only; the session extends the previous open conversation) — normally the LLM's own judgment, emitted as a response prefix marker, but a *Persona switch* always forces `BREAK` deterministically, overriding whatever the LLM tagged (TR-315): a conversation is attributed to one persona for consolidation, so a switch must always start a new one. |
